...
!     PCOEF, ARE CALCULATED.  THESE TERMS, UNLIKE THE NET FLUXES,
...
!       L,LP2            CURRENT & TOP LAYER BOUNDARY INDEX USED IN FIT.
...
!     DETERMINE FITTING COEFFICIENTS FOR THE FIRST LAYER.
      ...
      PCOEF(1,1)=0.
      PCOEF(2,1)=0.
      PCOEF(3,1)=COEF/(Z1-Z2)+COEF/(Z1-Z3)
      PCOEF(4,1)=COEF*(Z1-Z3)/((Z2-Z1)*(Z2-Z3))
      PCOEF(5,1)=COEF*(Z1-Z2)/((Z3-Z1)*(Z3-Z2))
...
!     DETERMINE FITTING COEFFICIENTS FOR THE SECOND LAYER.
      ...
      COEF=GCP/PM(2)
      PCOEF(1,2)=0.
      PCOEF(2,2)=COEF*(Z2-Z3)*(Z2-Z4)/((Z1-Z2)*(Z1-Z3)*(Z1-Z4))
      PCOEF(3,2)=COEF/(Z2-Z1)+COEF/(Z2-Z3)+COEF/(Z2-Z4)
      PCOEF(4,2)=COEF*(Z2-Z1)*(Z2-Z4)/((Z3-Z1)*(Z3-Z2)*(Z3-Z4))
      PCOEF(5,2)=COEF*(Z2-Z1)*(Z2-Z3)/((Z4-Z1)*(Z4-Z2)*(Z4-Z3))
...
!             WRITE OUT PRESSURE HEADER FOR COOLING RATES FILE.
     ...
     &          WRITE(ISPCCR,FMT=FRMT)LABEL,(DBLE(PM(I)),I=ILO,LP2)
          ...
          Z5=LOG(PM(LP2))
          COEF=GCP/PM(L)
          PCOEF(1,L)=COEF*(Z3-Z2)        *(Z3-Z4)*(Z3-Z5)               &
     &                  /((Z1-Z2)*(Z1-Z3)*(Z1-Z4)*(Z1-Z5))
          PCOEF(2,L)=COEF*(Z3-Z1)        *(Z3-Z4)*(Z3-Z5)               &
     &                  /((Z2-Z1)*(Z2-Z3)*(Z2-Z4)*(Z2-Z5))
          PCOEF(3,L)=COEF/(Z3-Z1)+COEF/(Z3-Z2)+COEF/(Z3-Z4)+COEF/(Z3-Z5)
          PCOEF(4,L)=COEF*(Z3-Z1)*(Z3-Z2)        *(Z3-Z5)               &
     &                  /((Z4-Z1)*(Z4-Z2)*(Z4-Z3)*(Z4-Z5))
          PCOEF(5,L)=COEF*(Z3-Z1)*(Z3-Z2)        *(Z3-Z4)               &
...
!     DETERMINE FITTING COEFFICIENTS FOR THE SECOND TO LAST LAYER.
      COEF=GCP/PM(L)
      PCOEF(1,L)=COEF*(Z3-Z2)*(Z3-Z4)/((Z1-Z2)*(Z1-Z3)*(Z1-Z4))
      PCOEF(2,L)=COEF*(Z3-Z1)*(Z3-Z4)/((Z2-Z1)*(Z2-Z3)*(Z2-Z4))
      PCOEF(3,L)=COEF/(Z3-Z1)+COEF/(Z3-Z2)+COEF/(Z3-Z4)
      PCOEF(4,L)=COEF*(Z3-Z1)*(Z3-Z2)/((Z4-Z1)*(Z4-Z2)*(Z4-Z3))
      PCOEF(5,L)=0.

!     DETERMINE FITTING COEFFICIENTS FOR THE LAST LAYER.
      COEF=GCP/PM(ML)
      PCOEF(1,ML)=COEF*(Z4-Z3)/((Z2-Z3)*(Z2-Z4))
      PCOEF(2,ML)=COEF*(Z4-Z2)/((Z3-Z2)*(Z3-Z4))
      PCOEF(3,ML)=COEF/(Z4-Z2)+COEF/(Z4-Z3)
      PCOEF(4,ML)=0.
      PCOEF(5,ML)=0.
...
!     WRITE OUT PRESSURE HEADER FOR COOLING RATES FILE.
     ...
     &                         '    CM-1   K DAY-1 / CM-1'
      ...
      RETURN
      END
      CHARACTER*1 FUNCTION PFRMT(PM)

!     FUNCTION USED TO DEFINE FORMAT FOR WRITING PM
      REAL PM
      DOUBLE PRECISION DPM
      DPM=DBLE(PM)
      IF(DPM.GE.999.9999995D0)THEN
          PFRMT='5'
      ELSEIF(DPM.GE.99.99999995D0)THEN
          PFRMT='6'
      ELSEIF(DPM.GE.9.999999995D0)THEN
          PFRMT='7'
      ELSEIF(DPM.GE..9999999995D0)THEN
          PFRMT='8'
      ELSE
          PFRMT='9'
      ENDIF
