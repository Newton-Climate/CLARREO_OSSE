...
!     NO NEED TO PRINT IF JMASS
      ...
      IF (IBCND.NE.1) WRITE(*,1020)  NTAU, (UTAU(LU), LU=1, NTAU)
      IF (.NOT.ONLYFL)                                                  &
...
!        IF (.NOT.LAMBER)  WRITE(*,1090) (HL(K), K=0, NSTR)
     ...
     &                                 TTEMP, TEMIS
      ...
      IF (PLANK )  WRITE(*,1190)
      IF (.NOT.PLANK)  WRITE(*,1200)
      YESSCT=0.0
      DO 10 LC=1, NLYR
         YESSCT=YESSCT + SSALB(LC)
         IF(PLANK)                                                      &
     &       WRITE(*,1210)  LC, DTAUC(LC), TAUC(LC), SSALB(LC),         &
     ...
     &                    OPRIM(LC), PMOM(1,LC)
10    CONTINUE
      IF(PLANK)  WRITE(*,1220) TEMPER(NLYR)
...
1020  FORMAT(I4,' USER OPTICAL DEPTHS :',10F10.4, /, (26X,10F10.4))
...
1210  FORMAT(I4, 2F10.4, F10.5, F12.5, 2F10.4, F10.5, F9.4,F14.3)
1220  FORMAT(85X, F14.3)
