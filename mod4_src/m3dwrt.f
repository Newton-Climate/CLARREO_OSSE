     ...
     &  IVXMIN,IVXMAX,IBNDWD,KNTRVL,SUBINT,IK,IKMAX,NATM,PRES,DENH2O,   &
     &  DENCO2,DENO3,SAER,XAER,GAER,SCLD,XCLD,GCLD,SRAIN,XRAIN,         &
     &  GRAIN,XMOL,LASCII,LFIRST,M3DGEN,M3DCNT,M3DDAT,M3DMOL,LNFLRT,    &
     &  FLRT)
...
!       M3DMOL   NAME OF MOLECULAR EXTINCTION DATA FILE FOR MOD3D.
      INTEGER I3DGEN,I3DCNT,I3DDAT,IHAZE,ICLD,M1,                       &
     &  IVXMIN,IVXMAX,IBNDWD,KNTRVL,IK,IKMAX,NATM
      REAL SUBINT(*),PRES(*),DENH2O(*),DENCO2(*),DENO3(*),              &
     &  SAER(*),XAER(*),GAER(*),SCLD(*),XCLD(*),GCLD(*),                &
     &  SRAIN(*),XRAIN(*),GRAIN(*),SMOL(*),XMOL(*)
      LOGICAL LASCII,LFIRST
      CHARACTER M3DGEN*(*),M3DCNT*(*),M3DDAT*(*),M3DMOL*(*)
      INTEGER LNFLRT
      CHARACTER FLRT*(*)
...
!         OPEN MOD3D GENERAL INFORMATION DATA FILE:
          OPEN(UNIT=I3DGEN,FILE=M3DGEN,STATUS='UNKNOWN')
          CLOSE(UNIT=I3DGEN,STATUS='DELETE')
          OPEN(UNIT=I3DGEN,FILE=M3DGEN,STATUS='NEW',FORM='FORMATTED')
...
!         AEROSOL, CLOUD, AND SPECTRAL DATA:
          WRITE(I3DGEN,'(/(I8,3X,A))')                                  &
     &      IHAZE,CHAZE(IHAZE),ICLD,CCLD(ICLD),M1,CTEMP(M1),            &
     &      IVXMIN,'MINIMUM SPECTRAL FREQUENCY [CM-1]',                 &
     &      IVXMAX,'MAXIMUM SPECTRAL FREQUENCY [CM-1]',                 &
     &      IBNDWD,'SPECTRAL STEP SIZE (BAND MODEL WIDTH) [CM-1]',      &
     &      KNTRVL-1,                                                   &
...
!         DELETE OLD MOD3D CONTINUA AND MOLECULAR ABSORPTION DATA FILES:
          OPEN(UNIT=I3DCNT,FILE=M3DCNT,STATUS='UNKNOWN')
          CLOSE(UNIT=I3DCNT,STATUS='DELETE')
          OPEN(UNIT=I3DDAT,FILE=M3DDAT,STATUS='UNKNOWN')
          CLOSE(UNIT=I3DDAT,STATUS='DELETE')

!         OPEN NEW MOD3D CONTINUA AND MOLECULAR ABSORPTION DATA FILES:
          IF(LASCII)THEN

!             ASCII DATA FILES:
              OPEN(UNIT=I3DCNT,FILE=M3DCNT,STATUS='NEW',                &
     &          FORM='FORMATTED')
              WRITE(FRMT(2:4),'(I3)')KNTRVL
              OPEN(UNIT=I3DDAT,FILE=M3DDAT,STATUS='NEW',                &
     &          FORM='FORMATTED')
          ELSE
...
!             BINARY DATA FILES:
     ...
     &          FORM='UNFORMATTED',ACCESS='DIRECT',RECL=LRECLN)
              LRECLN=IRECLN(KNTRVL,LNFLRT,FLRT)
              OPEN(UNIT=I3DDAT,FILE=M3DDAT,STATUS='NEW',                &
     &          FORM='UNFORMATTED',ACCESS='DIRECT',RECL=LRECLN)
          ENDIF
...
!         WRITE MOLECULAR EXTINCTION DATA:
          WRITE(I3DDAT,FRMT)(XMOL(INTRVL),INTRVL=1,KNTRVL)
      ELSE
...
!         WRITE MOLECULAR EXTINCTION DATA:
          NRECM=NRECM+1
          WRITE(I3DDAT,REC=NRECM)(XMOL(INTRVL),INTRVL=1,KNTRVL)
      ENDIF
      RETURN
      END
