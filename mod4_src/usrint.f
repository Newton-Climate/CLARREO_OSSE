...
!+---------------------------------------------------------------------+
     ...
     &         GC(MXCMU,MXCMU,*), GU(MXUMU,MXCMU,*), KK(MXCMU,*),       &
...
50          BNDINT=0.0
            ...
            ELSE IF (.NOT.NEGUMU)  THEN
              ...
              DO 60 JQ=NN+1, NSTR
60              WK(JQ)=EXP(-KK(JQ,NLYR)*(TAUCPR(NLYR)-TAUCPR(NLYR-1)))
              BNDDFU=0.0
              DO 80 IQ=NN, 1, -1
                 DFUINT=0.0
                 DO 70 JQ=1, NN
70                 DFUINT=DFUINT + GC(IQ,JQ,NLYR) * LL(JQ,NLYR)
                 DO 71 JQ=NN+1, NSTR
71                 DFUINT=DFUINT + GC(IQ,JQ,NLYR) * LL(JQ,NLYR) * WK(JQ)
                 IF (FBEAM.GT.0.0)                                      &
