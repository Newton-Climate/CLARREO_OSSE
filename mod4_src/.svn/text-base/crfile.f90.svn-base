...
!       CIRTYP   CIRRUS CLOUD TYPE OR NAME.
      CHARACTER*80 CFILE,CLDTYP,CIRTYP
...
!                REFERENCE WAVELENGTH [KM-1 M3 / G].
      ...
      CHARACTER*80 INPSTR
      REAL PFSUM,PFRAT,RENORM,LEGSUM,EXTWD,EXTIP

!     FUNCTIONS:
!       LENSTR   RETURNS STRING LENSTR AFTER TRUNCATING LEADING BLANKS.
!       NUNIT    RETURNS AN UNUSED FILE UNIT NUMBER.
!       UPCASE   RETURNS UPPER CASE STRING.
      INTEGER LENSTR,NUNIT
      CHARACTER*80 UPCASE
      EXTERNAL LENSTR,NUNIT,UPCASE
...
!         ZENITH ANGLE INTEGRATION (ASSUME EXPONENTIAL VARIATION):
     ...
     &              ' was read in for cloud type,',CLDTYP(1:LENTYP)
              ...
              IF(ABS(PFRAT-1).GT..001)THEN
                  PFSUM=PFSUM+(CLDCOS(IANGM1)-CLDCOS(ICLDAN))/LOG(PFRAT)&
     &              *(CLDPF(IANGM1,ICLDWV)-CLDPF(ICLDAN,ICLDWV))
              ELSE
                  PFSUM=PFSUM+2*(CLDCOS(IANGM1)-CLDCOS(ICLDAN))         &
...
!         ZENITH ANGLE INTEGRATION (ASSUME EXPONENTIAL VARIATION):
     ...
     &              ' was read in for cloud type,',CIRTYP(1:LENTYP)
              ...
              IF(ABS(PFRAT-1).GT..001)THEN
                  PFSUM=PFSUM+(CIRCOS(IANGM1)-CIRCOS(ICIRAN))/LOG(PFRAT)&
     &              *(CIRPF(IANGM1,ICIRWV)-CIRPF(ICIRAN,ICIRWV))
              ELSE
                  PFSUM=PFSUM+2*(CIRCOS(IANGM1)-CIRCOS(ICIRAN))         &
