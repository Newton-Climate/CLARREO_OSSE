     ...
     &                  ZEN_INPUT, A_FLAG, FULL_ATM,V1_INPUT,V2_INPUT,  &
     ...
     &                  CTHIK_INPUT,CEXT_INPUT,AC_WAVELEN,FILE_NAME,    &
     &                  WVLNGTH_LRES,RADIANCE_LRES,WV_HRES,             &
     &                  RADIANCE_HRES,NUM_LRES,NUM_HRES,NUM_AC_WVL,     &
     &                  SOLAR_FLUX,ICE_FLAG,SNOW_FLAG,PFS,PHASE_WVLS,   &
     &                  PHASE_ANGLES,DIFFUSE_FLUX,SOLAR_ZENITH,         &
     &                  IN_LENGTH,DV_INPUT,PSIPO_INPUT,                 &
...
!DRF
      ...
      LOGICAL A_FLAG,LAND_FLAG,C_FLAG,ICE_FLAG,SNOW_FLAG
...
!       FRMT     FORMAT STRING.
      ...
      !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      !WRITE OUT MODTRAN INPUT ARGUMENTS
      !!WRITE(*,*) "TPTEMP_INPUT = ",TPTEMP_INPUT
      !!WRITE(*,*) "GNDALT_INPUT = ",GNDALT_INPUT
      !!WRITE(*,*) "CO2MX_INPUT = ",CO2MX_INPUT
      !!WRITE(*,*) "DAY_INPUT = ",DAY_INPUT
      !!WRITE(*,*) "LATITUDE_INPUT = ",LATITUDE_INPUT
      !!WRITE(*,*) "LONGITUDE_INPUT = ",LONGITUDE_INPUT
      !!WRITE(*,*) "GMT_INPUT = ",GMT_INPUT
      !!WRITE(*,*) "A_FLAG = ",A_FLAG
      !WRITE(*,*) "FULL_ATM = ",FULL_ATM
      !!WRITE(*,*) "V1_INPUT = ",V1_INPUT
      !!WRITE(*,*) "V2_INPUT = ",V2_INPUT
      !!WRITE(*,*) "LAND_FLAG = ",LAND_FLAG
      !!WRITE(*,*) "BRDF_LEN = ",BRDF_LEN
      !!WRITE(*,*) "BRDF_WVL = ",BRDF_WVL
      !WRITE(*,*) "BRDF_PARAM = ",BRDF_PARAM
      !!WRITE(*,*) "BRDF_FLAG = ",BRDF_FLAG
      !!WRITE(*,*) "C_FLAG = ",C_FLAG
      !!WRITE(*,*) "NUM_LEVS = ",NUM_LEVS
      !WRITE(*,*) "M_AEROSOL = ",M_AEROSOL
      !WRITE(*,*) "O_AEROSOL = ",O_AEROSOL
      !WRITE(*,*) "M_CLOUD_LIQ = ",M_CLOUD_LIQ
      !WRITE(*,*) "M_CLOUD_ICE = ",M_CLOUD_ICE
      !WRITE(*,*) "WVL_CLD = ",WVL_CLD
      !WRITE(*,*) "EXT_LIQ_CLD = ",EXT_LIQ_CLD
      !WRITE(*,*) "SSA_LIQ_CLD = ",SSA_LIQ_CLD
      !WRITE(*,*) "ASYM_LIQ_CLD = ",ASYM_LIQ_CLD
      !WRITE(*,*) "EXT_ICE_CLD = ",EXT_ICE_CLD
      !WRITE(*,*) "SSA_ICE_CLD = ",SSA_ICE_CLD
      !WRITE(*,*) "ASYM_ICE_CLD = ",ASYM_ICE_CLD
      !WRITE(*,*) "Z_CLD_VAL = ",Z_CLD_VAL
      !!WRITE(*,*) "AC_WAVELEN = ",AC_WAVELEN
      !!WRITE(*,*) "FILE_NAME = ",FILE_NAME
      !!WRITE(*,*) "NUM_AC_WVL = ",NUM_AC_WVL
      !!WRITE(*,*) "ICE_FLAG = ",ICE_FLAG
      !!WRITE(*,*) "SNOW_FLAG = ",SNOW_FLAG
      !WRITE(*,*) "PFS = ",PFS
      !!WRITE(*,*) "PHASE_WVLS = ",PHASE_WVLS
      !!WRITE(*,*) "PHASE_ANGLES = ",PHASE_ANGLES
      !!WRITE(*,*) "IN_LENGTH= ",IN_LENGTH
      !!WRITE(*,*) "DV_INPUT= ",DV_INPUT
...
!     CARD 1:
     ...
     &      ' ****************************************************'
          IF (DRF_FLAG) THEN
              ...
              !IF(LAND_FLAG.OR.SNOW_FLAG.OR.ICE_FLAG) THEN
                 SURREF = 'BRDF'
              !ELSE
              !   SURREF = 'LAMBER'
              !ENDIF
...
!         CARD 2A MODEL CLOUDS
     ...
     &          NCRSPC,CWAVLN,CCOLWD,CCOLIP,CHUMID,ASYMWD,ASYMIP
              ELSE
                !WRITE(*,*) "NEED CARD 2A VALS"
                CLD_INDEX_LOW = 0
                CLD_INDEX_HIGH = 0
                DO CLD_INDEX=1,NUM_LEVS-1
                   IF (CLD_INDEX_LOW.EQ.0) THEN
                     IF (M_CLOUD_LIQ(CLD_INDEX).GT.0.0000001 .OR.       &
     &                   M_CLOUD_ICE(CLD_INDEX).GT.0.0000001) THEN
                        CLD_INDEX_LOW = CLD_INDEX
                     ENDIF
                   ENDIF
                   IF (M_CLOUD_LIQ(CLD_INDEX).GT.0.0000001 .OR.         &
     &                 M_CLOUD_ICE(CLD_INDEX).GT.0.0000001) THEN
                      CLD_INDEX_HIGH  = CLD_INDEX
                   ENDIF
                END DO

                !CALT = Z_CLD_VAL(CLD_INDEX_LOW)-GNDALT_INPUT
                !CTHIK = CTHIK_INPUT
                !CEXT = CEXT_INPUT
...
!             CARD 3A2:
		  ...
		  !PARM2 = 244. !!!DRF LONGITUDE_INPUT
		  PARM1 = PSIPO_INPUT
		  PARM2 = ZEN_INPUT
		  WRITE(*,*) 'SOLAR ZENITH = ',ZEN_INPUT
		  WRITE(*,*) 'SOLAR AZIMUTH = ',PSIPO_INPUT
		  PARM3 = 0.
		  PARM4 = 0.
		  TIME = 13.5
		  PSIPO = 0.
...
!DRF      IF(.NOT.RFLECT(SURREF,DATDIR,LENDAT))THEN
      ...
      !WRITE(*,*) 'SNOW_FLAG in driver = ',SNOW_FLAG
         IF(.NOT.RFLECT(SURREF,DATDIR,LENDAT,LAND_FLAG,ICE_FLAG,        &
     &                  SNOW_FLAG,BRDF_LEN,BRDF_WVL,BRDF_PARAM))THEN
...
!	      LOPEN = .TRUE.
     ...
     &      LNFLRT,FLRT,NUM_LEVS,BB_UPDIFFUSE,BB_DNDIFFUSE,BB_DNDIRECT, &  
     &      FLX_UPDIFFUSE,FLX_DNDIFFUSE,FLX_DNDIRECT)
      ELSE
          CALL TRANS(IPH,ISOURC,IDAY,ANGLEM,                            &
