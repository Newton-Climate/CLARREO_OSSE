     ...
     &  H2OCOL,O3COL,LYMOL,REARTH,TP5_FLAG,TP5_FILE,TP5_LINENUMBER)
...
!         CARD 2C:  USER SUPPLIED ATMOSPHERIC PROFILE
     ...
     &          ML,IRD1,IRD2,HMODEL(MODEL),REARTH,NMOLYC
              ELSE
                 READ(TP5_FILE(TP5_LINENUMBER),'(3I5,A20,F10.0,I5)',    &
     &                IOSTAT=IOS)                                       &
     &                ML,IRD1,IRD2,HMODEL(MODEL),REARTH,NMOLYC
...
!                     READ IN "Y-SPECIES" NAMES (CARD 2CY):
                      ...
                      ELSE
                        READ(TP5_FILE(TP5_LINENUMBER),'((8A10))')       &
     &                    (YNAME(IMOLY),IMOLY=1,NMOLYC)
                        TP5_LINENUMBER=TP5_LINENUMBER+1
                      ENDIF
                  ELSE
...
!                     SWAP MAPY_D, MAPY_F, ITBY, CNAMEY, DOPY AND AMWTY:
     ...
     &          ' *****  APLUS OPTION WILL BE IGNORED  *****'
              ...
              APLUS='  '
          ENDIF
          IF(IVSA.EQ.1)CALL RDNSM(TP5_FLAG,TP5_FILE,TP5_LINENUMBER) 
      ENDIF
      MARIC1=0
      MARK=0
      JPRT=0
...
!         ROUTINE AERNSM; TEMPORARILY SET ICLD AND RAINRT TO ZERO.
     ...
     &                TP5_FLAG,TP5_FILE,TP5_LINENUMBER)
          ICLD=ICLDSV
          RAINRT=RAINSV
          CALL CRDRIV(TP5_FLAG,TP5_FILE,TP5_LINENUMBER)
      ELSE
          CALL AERNSM(JPRT,GNDALT,MARIC1,MARK,ICH,LMODEL,               &
     &                TP5_FLAG,TP5_FILE,TP5_LINENUMBER)
      ENDIF
