...
!         BIDIRECTIONAL REFLECTANCE DISTRIBUTION FUNCTION (BRDF):
     ...
     &                 '(I1,F9.0,F9.0,A)',IOSTAT=IOTEST)NSURF,AATEMP,   &
     &            DH2O,DTF
...
!         READ SURFACE SPECTRAL ALBEDO FILE NAME:
          ...
          ELSE
              ...
              ELSE
                READ(TP5_FILE(TP5_LINENUMBER),CFRMT,IOSTAT=IOTEST)SALBFL
                TP5_LINENUMBER=TP5_LINENUMBER+1
              ENDIF 
          ENDIF
          IF(IOTEST.NE.0)THEN
...
!             THIS SURFACE IS ALSO USED FOR THE AREA-AVERAGED GROUND.
              IF(.NOT.LJMASS)WRITE(IPR,'(/A)')                          &
     &          ' Uniform ground surface (NSURF=1)'
              RFLECT=RDLAMB(1,NLOS,.TRUE.,SALBFL,TP5_FLAG,TP5_FILE,     &
     &                      TP5_LINENUMBER)
          ELSE
...
!             A SEPARATE AREA-AVERAGED GROUND SURFACE WILL BE DEFINED.
     ...
     &          'averaged ground surface is distinct from the imaged',  &
     &          ' pixel with temperature (AATEMP) equal',AATEMP,'K.'
              RFLECT=RDLAMB(1,NLOS,.FALSE.,SALBFL,TP5_FLAG,TP5_FILE,    &
     &                      TP5_LINENUMBER)
          ENDIF
          IF(RFLECT)THEN
...
!             DEFINE THE AREA-AVERAGED SURFACE REFLECTANCE:
              IF(NSURF.EQ.2)THEN
                  RFLECT=RDLAMB(MLOSP1,NLOS,.TRUE.,SALBFL,TP5_FLAG,     &
...
!         READ IN LAMBERTIAN SURFACE IDENTIFIER:
          ...
          ELSE
              IF (TP5_FLAG) THEN
                READ(IRD,'(A80)')CSALB
              ELSE
                READ(TP5_FILE(TP5_LINENUMBER),'(A80)')CSALB
                TP5_LINENUMBER=TP5_LINENUMBER+1
              ENDIF 
...
!             READ SPECTRAL ALBEDO:
              ...
              READ(TP5_FILE(TP5_LINENUMBER),'(2F10.6)')DUMMY1,DUMMY2
              TP5_LINENUMBER=TP5_LINENUMBER+1
              SALB(NEXTWV)=DUMMY2
...
!         READ IN BRDF IDENTIFIER:
          ...
          ELSE
              IF (TP5_FLAG) THEN
                READ(IRD,'(A80)')CBRDF
              ELSE
                READ(TP5_FILE(TP5_LINENUMBER),'(A80)')CBRDF
                TP5_LINENUMBER=TP5_LINENUMBER+1
              ENDIF 
          ENDIF
...
!             COX-MUNK BRDF FROM 6S
              IF(.NOT.LJMASS)THEN
                  IF(.NOT.GSURF)THEN
                      WRITE(IPR,'(/A,I3)')                              &
     &                  ' Cox-Munk Model BRDF for Surface No.',ILOS0    &
                  ELSEIF(ILOS0.EQ.1)THEN
                      WRITE(IPR,'(/2A)')' Cox-Munk model BRDF for'      &
     &                  //' Uniform (Homogeneous) Surface.'
                  ELSE
                      WRITE(IPR,'(/2A)')' Cox-Munk Model BRDF for'      &
     &                  //' Area-Averaged Surface.'
                  ENDIF
              ENDIF
              RDBRDF=GTBRDF(2,REFCXMK,ILOS0,NLOS,GSURF,DH2O,MLTRFL,     &
     &                        TP5_FLAG,TP5_FILE,TP5_LINENUMBER)
...
!     READ IN NUMBER OF WAVELENGTH GRID POINTS AND SURFACE NORMAL:
      ...
      ELSE
          ...
          ELSE
             READ(TP5_FILE(TP5_LINENUMBER),*,IOSTAT=IOTEST)             &
     &            NWVSRF(ILOS0),SURFZN,SURFAZ
             TP5_LINENUMBER=TP5_LINENUMBER+1
          ENDIF
      ENDIF
      IF(IOTEST.NE.0)THEN
...
!C OUTPUT: brdfint(j,k)=the total reflectance of the sea water
      ...
      ELSE
         PWS = 2.0
      ENDIF
      PAW = 0.  !WIND-DIRECTION (reflectance is a weak function of wind direction)
      XSAL = 35.0
      PCL = 0.2
      PWL = PARAMS(1)
      PHI2 = PHI
