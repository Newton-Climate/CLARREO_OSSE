      SUBROUTINE FNAMES(NMFLRT,CLRT,SPFLUX,DBOUT,REFPTH,FILE_NAME,      &
     &                 FLENGTH)
...
!       REFPTH   NAME OF REFRACTIVE PATH INPUT FILE.
      ...
      INTEGER NMFLRT
      CHARACTER*120 FILE_NAME  !DRF MODIFICATION
      INTEGER FLENGTH
...
!             MOD5ROOT.IN or mod5root.in EXISTS, FIND FIRST ROOT NAME.
              ...
              !WRITE(*,*) 'FILE_LENGTH = ',FLENGTH
              FLRT = FILE_NAME(1:FLENGTH)
              IF(IOS.EQ.0)THEN
                  LNFLRT=LENSTR(FLRT)
...
!         TRUNCATE ".TP5" IF PRESENT IN FLRT:
      ...
      IF(LNFLRT.LE.0)THEN
          ...
          REFPTH='rfracpth.dat'
      ELSE
          WRITE(*,*) 'FNAME = ',FLRT(1:LNFLRT)
          CALL OPNFL(IPR,0,FLRT(1:LNFLRT)//'.tp6','UNKNOWN',            &
     &      'FORMATTED','FNAMES')
          !CALL OPNFL(IPR,'/dev/null','OLD')
!DRF          CALL OPNFL(IRD,0,FLRT(1:LNFLRT)//'.tp5','OLD','FORMATTED',    &
!DRF     &      'FNAMES')
          CALL OPNFL(IPU,0,'/tmp/'//FLRT(1:LNFLRT)//'.tp7','UNKNOWN',   &
     &      'FORMATTED','FNAMES')
