...
!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      INTEGER DAY_INPUT
      REAL ZEN_INPUT,V1_INPUT,V2_INPUT,DV_INPUT
      REAL PSIPO_INPUT
      REAL LATITUDE_INPUT,LONGITUDE_INPUT
      INTEGER NUM_LEVS
      INTEGER NUM_LRES,NUM_HRES,NUM_AC_WVL
      LOGICAL A_FLAG,LAND_FLAG,C_FLAG,ICE_FLAG,TP5_FLAG
      REAL FULL_ATM(26,28)
      INTEGER BRDF_LEN
      REAL BRDF_WVL(7)
      REAL BRDF_PARAM(7,3)
      ...
      LOGICAL DEBUG_FLAG
      REAL M_AEROSOL(26,4),O_AEROSOL(42,12)
      REAL M_CLOUD_LIQ(26-1)
      REAL M_CLOUD_ICE(26-1)
      REAL WVL_CLD(24),EXT_LIQ_CLD(24)
      REAL SSA_LIQ_CLD(24),ASYM_LIQ_CLD(24)
      REAL EXT_ICE_CLD(24),SSA_ICE_CLD(24),ASYM_ICE_CLD(24)
      REAL Z_CLD_VAL(26-1)

      REAL AC_WAVELEN(42) !WAVELENGTHS FOR AEROSOL/CLOUD OPT. PROPERTIES
      CHARACTER*120 FILE_NAME
      INTEGER IN_LENGTH !LENGTH OF FILE
      REAL WVLNGTH_LRES(450), RADIANCE_LRES(450)
      REAL WV_HRES(3100), RADIANCE_HRES(3100)
      REAL SOLAR_FLUX(450),DIFFUSE_FLUX(450),SOLAR_ZENITH
      REAL PFS(50,15,4)
      REAL PHASE_WVLS(15),PHASE_ANGLES(50)
      REAL BB_UPDIFFUSE(26)
      REAL BB_DNDIFFUSE(26) 
      REAL BB_DNDIRECT(26)
      REAL FLX_UPDIFFUSE(26,450)
      REAL FLX_DNDIFFUSE(26,450) 
      REAL FLX_DNDIRECT(26,450)
...
!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      TEST_TP5 = 1
      TP5_FLAG = .FALSE.
      CALL OPNFL(TEST_TP5,0,                                            &
     &          'modtran_out_b30.042a.cam2.h0.2000-01.82.1-debug.tp5',  &
     &          'OLD','FORMATTED','Modtrn')
      I=1
      IOS = 0
      DO WHILE(IOS.EQ.0)
        READ(TEST_TP5,'(A80)',IOSTAT=IOS) TP5_FILE(I)
        I=I+1
      ENDDO
      !DO I=1,698
      !  WRITE(*,*) 'LINE INFO = ',TP5_FILE(I)
      !ENDDO
      CLOSE(TEST_TP5,STATUS='KEEP')

      CALL DRIVER(FILE_NAME,TP5_FLAG,TP5_FILE,NUM_LEVS,                 &
     &                  WVLNGTH_LRES,RADIANCE_LRES,WV_HRES,             &
     &                  RADIANCE_HRES,NUM_LRES,NUM_HRES,NUM_AC_WVL,     &
     &                  SOLAR_FLUX,DIFFUSE_FLUX,SOLAR_ZENITH,           &
