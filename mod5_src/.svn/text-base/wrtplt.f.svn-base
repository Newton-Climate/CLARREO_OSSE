     ...
     &                  NUM_HRES,WV_HRES,RADIANCE_HRES,ICOUNT2) 
...
!      OUTPUT ARGUMENTS
!       WV_HRES   WAVELENGTH GRID AT NATIVE RESOLUTION
!       RADIANCE_HRES  RADIANCE VALUES AT NATIVE RESOLUTION (W/sr/cm^2/cm^-1)
!       NUM_HRES       SIZE OF WV_HRES,RADIANCE_HRES
!       ICOUNT2        INPUT VAL INDICATING INDEX OF WV_HRES,RADIANCE_HRES
...
!!!BEGIN DRF MODIFICATION
              IF(ICOUNT2.LE.NUM_HRES) THEN 
                WV_HRES(ICOUNT2) = 1000.*ALAM
                RADIANCE_HRES(ICOUNT2) = REAL(CONVRT*SUMT(1))
              ENDIF 
              !WRITE(*,*) 'ICOUNT2 = ',ICOUNT2
              !WRITE(*,*) 'WV = ',WV_HRES(ICOUNT2)
              !WRITE(*,*) 'RADIANCE_HRES = ',RADIANCE_HRES(ICOUNT2)
              !IF (ICOUNT2.GT.20) THEN
              !   STOP
              !ENDIF
!!!END DRF MODIFICATION
          ELSEIF(XFLAG.EQ.'M')THEN
          ...
          ELSE
              ...
              IF(ICOUNT2.LE.NUM_HRES) THEN 
                WV_HRES(ICOUNT2) = VCEN
                RADIANCE_HRES(ICOUNT2) = REAL(SUMT(1))
              ENDIF 
          ENDIF
          DO ILOS=1,NLOS
