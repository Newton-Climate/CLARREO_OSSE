      SUBROUTINE DRIVER(FILE_NAME,FLENGTH,TP5_FLAG,TP5_FILE,NUM_LEVS,   &
...
!       FILE_SCRATCH SCRATCH FILENAME (DRF MODIFICATION)
      INTEGER IPH
      REAL G
      COMMON/CARD3A/IPH,G
      CHARACTER FILE_SCRATCH*(50)
...
!     DEFINE FILE NAMES AND OPEN FILES:
   ...
   10 CONTINUE
      ...
      CALL FNAMES(NMFLRT,CLRT,SPFLUX,DBOUT,REFPTH,FILE_NAME,FLENGTH)
...
!!!BEGIN DRF MODIFICATION
     ...
     &         TP5_FLAG,TP5_FILE,TP5_LINENUMBER)
      !READ(TP5_FILE(TP5_LINENUMBER),'(A80)') DUMMY
      !WRITE(*,*) 'DUMMY = ',DUMMY
!!!END DRF MODIFICATION
!DRF      CALL CD1(NLOS,TPTMPS,APPREF,LMODEL,LYMOL,INITC1,KPRINT,CLRT)
...
!     CARD 2A CLOUD MODEL:
      CALL CD2A(LMODEL,TP5_FLAG,TP5_FILE,TP5_LINENUMBER)

!     CARD 2B VERTICAL STRUCTURE ALGORITHM:
      CALL CD2B(TP5_FLAG,TP5_FILE,TP5_LINENUMBER)
...
!     CARD 2E USER-SUPPLIED AEROSOL EXTINCTION, ABSORPTION & ASYMMETRY:
      CALL CD2E(TP5_FLAG,TP5_FILE,TP5_LINENUMBER)
...
!     CARD 3 GEOMETRY IPUTS:
      CALL CD3(1,REFPTH,TP5_FLAG,TP5_FILE,TP5_LINENUMBER)
      
!     CARD 3A SOLAR GEOMETRY INPUTS:
      !DRF MODIFICATION CALL CD3A(ANGLEM,IDAY,ISOURC)
      FILE_SCRATCH = 'TEST_SCRATCH' !DRF TEMPORARY
      CALL CD3A(ANGLEM,IDAY,ISOURC,FILE_SCRATCH,                        &
     &         TP5_FLAG,TP5_FILE,TP5_LINENUMBER)
...
!     COMPUTE COLUMN DENSITY DATA:
     ...
     &  LENNSV(1),IKMAXS,KNTRVL,ICH,LNFLRT,FLRT)) THEN !DRF
      STOP
...
!         READ REMAINDER OF INPUTS AFTER A GEOMETRY ERROR DETECTED.
          CALL GEOERR(NLOS,LENDAT,LNFLRT,IRPTSV,FLRT,TP5_FLAG,TP5_FILE, &
     &                TP5_LINENUMBER)
          GOTO 60
      ENDIF
...
!         COMPUTE COLUMN DENSITY DATA:
     ...
     &      LENNSV(ILOS),IKMAXS,KNTRVL,ICH,LNFLRT,FLRT))THEN
...
!             READ REMAINDER OF INPUTS AFTER A GEOMETRY ERROR DETECTED.
              ...
              CALL GEOERR(NLOS,LENDAT,LNFLRT,IRPTSV,FLRT,TP5_FLAG,      &
     &                    TP5_FILE,TP5_LINENUMBER)
              GOTO 60
          ENDIF
...
!     SURFACE REFLECTANCE MODEL:
      AATEMP=0.
      IF(.NOT.RFLECT(SURREF,DATDIR,LENDAT,NLOS,DH2O,MLTRFL,             &
     &         TP5_FLAG,TP5_FILE,TP5_LINENUMBER))THEN
...
!         USE SOLAR IRRADIANCE DATA STORED IN /SOL05/:
     ...
     &      GROUND,KNTRVL,NLOS,LNFILT,GETS05,APPREF,DH2O,MLTRFL,        &
     &      WV_HRES,RADIANCE_HRES,NUM_HRES,SOLAR_FLUX,NUM_LRES,         &
     &      DIFFUSE_FLUX,LNFLRT,FLRT,NUM_LEVS,BB_UPDIFFUSE,BB_DNDIFFUSE,&
     &      BB_DNDIRECT,FLX_UPDIFFUSE,FLX_DNDIFFUSE,FLX_DNDIRECT)
      ELSEIF(RESCHR.EQ.'15' .AND. .NOT.LSUN)THEN

!         USE SOLAR IRRADIANCE DATA STORED IN /SOL15/:
          CALL TRANS(RESCHR,IPH,ISOURC,IDAY,ANGLEM,                     &
     &      GROUND,KNTRVL,NLOS,LNFILT,GETS15,APPREF,DH2O,MLTRFL,        &
     &      WV_HRES,RADIANCE_HRES,NUM_HRES,SOLAR_FLUX,NUM_LRES,         &
     &      DIFFUSE_FLUX,LNFLRT,FLRT,NUM_LEVS,BB_UPDIFFUSE,BB_DNDIFFUSE,&
     &      BB_DNDIRECT,FLX_UPDIFFUSE,FLX_DNDIFFUSE,FLX_DNDIRECT)
...
!         USE SOLAR IRRADIANCE DATA READ INTO /SOLP1/:
          CALL TRANS(RESCHR,IPH,ISOURC,IDAY,ANGLEM,                     &
     &      GROUND,KNTRVL,NLOS,LNFILT,GETSP1,APPREF,DH2O,MLTRFL,        &
     &      WV_HRES,RADIANCE_HRES,NUM_HRES,SOLAR_FLUX,NUM_LRES,         &
     &      DIFFUSE_FLUX,LNFLRT,FLRT,NUM_LEVS,BB_UPDIFFUSE,BB_DNDIFFUSE,&
     &      BB_DNDIRECT,FLX_UPDIFFUSE,FLX_DNDIFFUSE,FLX_DNDIRECT)
      ELSE
...
!         USE SOLAR IRRADIANCE DATA CURRENTLY IN /SOL01/:
          CALL TRANS(RESCHR,IPH,ISOURC,IDAY,ANGLEM,                     &
     &      GROUND,KNTRVL,NLOS,LNFILT,GETS01,APPREF,DH2O,MLTRFL,        &
     &      WV_HRES,RADIANCE_HRES,NUM_HRES,SOLAR_FLUX,NUM_LRES,         &
     &      DIFFUSE_FLUX,LNFLRT,FLRT,NUM_LEVS,BB_UPDIFFUSE,BB_DNDIFFUSE,&
     &      BB_DNDIRECT,FLX_UPDIFFUSE,FLX_DNDIFFUSE,FLX_DNDIRECT)
...
!     DEGRAD USING INFORMATION IN THE SCRATCH FILE TAPE7.SCR
      IF(DODGRD)THEN
          DO ILOS=1,NLOS
!DRF              CALL DGRD(DEGALL,I_SCAN,FWSCAN,                           &
!DRF     &          LSKIP,LNFLRT,FLRT,ILOS,NLOS)
!!!BEGIN DRF MODIFICATION
              CALL DGRD(DEGALL,I_SCAN,FWSCAN,                           &
     &          LSKIP,LNFLRT,FLRT,ILOS,NLOS,WVLNGTH_LRES,RADIANCE_LRES, &
     &          NUM_LRES)
!!!END DRF MODIFICATION
          ENDDO
      ELSE
...
!     INITIATE NEXT RUN:
      ...
      SUBROUTINE GEOERR(NLOS,LENDAT,LNFLRT,IRPTSV,FLRT,TP5_FLAG,        &
     &                 TP5_FILE,TP5_LINENUMBER)
...
!     SURFACE REFLECTANCE MODEL:
     ...
     &      ' input SURREF = "',SURREF,'" was unsuccessful.'
      ...
      ENDIF
      CALL CARD5(LNFLRT,FLRT,TP5_FLAG,TP5_FILE,TP5_LINENUMBER)
      IRPTSV=IRPT
      IRPT=ABS(IRPT)
