      SUBROUTINE I_LOOP(VBAND5,NO_WRT,IBINPT,VCEN,NLOS,SUMTMS,          &
...
!     LOOP LOOPS OVER ISAAC MULTIPLE SCATTER PATH SEGMENTS AT EACH
!     SPECTRAL FREQUENCY.
      IMPLICIT NONE
...
!       GROUND   LOGICAL FLAG, TRUE IF LINE OF SIGHT INTERSECTS GROUND.
      ...
      REAL VBAND5,VCEN,S0,PTHSCL(NLOS),SURSCL(NLOS)
      LOGICAL NO_WRT,LVBND5,GROUND(NLOS)

!     OUTPUT ARGUMENTS:
!       SUMTMS   ISAACS PATH THERMAL SCATTER [W CM-2 SR-1 / CM-1].
...
!       TRNLOS   LINE-OF-SIGHT COMBINED SPECIES TRANSMITTANCE [TX(9)].
     ...
     &  SUMSSS(NLOS),TSNOBS(NLOS),TSNREF(NLOS),TRNLOS(NLOS),            &
     &  SUMTMS(NLOS)
...
!       LBMFLG   FLAG, TRUE IF CENTER OR TAIL DATA FOR CURRENT FREQ BIN.
     ...
     &  NFRNTL,NFRNTH,ILOS,ITLSUB,IEXT
      REAL STORE,THMLAY,SOLLAY,WTTRAN,TRNOLD,TRNNEW,TX9LAY,TX9CUM,TMOL, &
     &  FACTOR,PFMOL0,PFMOL2,P2RAY,TX14SV,TX15SV,XPART,XPART0,GREY,     &
     &  SURFAC,SSAP,O3TRAN,T_CH4,UNIF,TRACE,TRANXY,DBACK,UBACKS,DBACKS, &
     &  UFRNT,DFRNT,UFRNTS,DFRNTS,UBACK,THMDIF,SOLDIF,BNCOEF,DFCOEF,    &
     &  THMSCT,OPTDEP,B0,OMEGA,UP,DN,THMBND,SOLBND,BLAYER,BBOUND
...
!       RMSCUM   CUMULATIVE PATH MS RADIANCE K-DATA [W SR-1 CM-2 / CM-1]
      REAL RADCUM(MXKSUB),RMSCUM(MXKSUB)
...
!         SKIP THE MULTIPLE SCATTERING CALCULATION:
          READ(ISCRCH)JNTRVL,(COSBAR(ISEG),GTSCAT(0,ISEG),              &
     &      (TAUT(INTRVL,ISEG),INTRVL=1,JNTRVL),ISEG=1,ML-1),           &
     &      ((UPF(INTRVL,ISEG),DNF(INTRVL,ISEG),UPFS(INTRVL,ISEG),      &
     &      DNFS(INTRVL,ISEG),INTRVL=1,JNTRVL),ISEG=1,ML)
...
!                 SENSOR IS IN SHADE IF WSPTHM IS NEGATIVE:
                  CALL SHADE(IPH,MLOSP1,ISEG,1,JNTRVL,VCEN,PFMOL0,      &
     &              PFMOL2,TSNOBS(1),TSNREF(1),SUMSSS(1),RADCUM)
              ENDIF
   10         CONTINUE
...
!                 VIS/NIR CH4:
                  ...
                  T_CH4=EXP                                             &
     &              (-(STORE+FRAC5*(SPCMLO(NSEG5-2,ISEG,IPATH)-STORE)))

!                 H2-H2 AND H2-HE DIMERS:
                  STORE=SPCMHI(NSEG5-1,ISEG,IPATH)
                  TX(MEXTX-1)=EXP                                       &
     &              (-(STORE+FRAC5*(SPCMLO(NSEG5-1,ISEG,IPATH)-STORE)))
                  STORE=SPCMHI(NSEG5,ISEG,IPATH)
                  TX(MEXTX)=EXP                                         &
     &              (-(STORE+FRAC5*(SPCMLO(NSEG5,ISEG,IPATH)-STORE)))
...
!                 NO CORRELATED-K:
                  IF(MODTRN)CALL BMOD(MLOSP1,ISEG,NSEG(MLOSP1),         &
...
!                         IF A PATH TRANSMITTANCE THROUGH "ISEG"
!                         SEGMENTS IS EXCEEDINGLY SMALL OR THE
!                         GROUND- TO-LEVEL VERTICAL TRANSMITTANCE IS
!                         RELATIVELY SMALL AND BEGUN TO INCREASE, SET
!                         THE TRANSMITTANCE TO EXACTLY ZERO SO THAT
!                         THE MOLECULAR OPTICAL DEPTH OF THE CURRENT
...
!                         3-PARAMETER CURTIS-GODSON HAS FAILED FOR O3:
                          O3MAX=TX(31)-O3TRAN
                          WRITE(IPR,'(A,F9.2,2(A,/21X),3(A,F10.7),      &
     &                      /21X,2A)')' Warning from LOOP:  At',VCEN,   &
     &                      ' CM-1, the 3-parameter Curtis-Godson'//    &
     &                      ' formulation is not',' producing a'//      &
     &                      ' monotonic curve-of-growth.  The O3',      &
     &                      ' transmittance increased',O3MAX,' from',   &
     &                      O3TRAN,' to',TX(31),'Run the Correlated-k'  &
...
!                     ARE INACCURATE AND ARE SET TO ZERO.
                      ...
                      CALL SSRAD(IPH,MLOSP1,ISEG,IPATH,VCEN,S0,         &
     &                  PFMOL0,PFMOL2,TMOL,SSAP,TX(1),                  &
...
!                 CK SOLAR PATH:
                  ...
                  CALL SSCORK(.FALSE.,IPH,MLOSP1,ISEG,IPATH,JNTRVL,     &
     &              VCEN,S0,PFMOL0,PFMOL2,SSAP,TX(1),TSNOBS(1),         &
     &              SUMSSS(1),RADCUM)
                  GOTO(10,20),IPATH
              ELSE
...
!                         OF BIGEXP IS USED.
                          TAUT(1,ISEG)=XPART+MIN(BIGEXP,                &
     &                      (TAUT(1,ISEG-1)-XPART0)*TX(15)/TX15SV)
                      ENDIF
                      TX15SV=TX(15)
                      XPART0=XPART
                  ELSE
...
!                 TO EXCEED TAUT; PROTECT AGAINST THIS ANOMALY.
                  ...
                  ENDIF
              ELSE
                  TCONT(ISEG)=TX(12)+TX(13)+TX(15)+TX(75)+TX(77)
              ENDIF
...
!*********** SHOULD SURSCL BE AN ARRAY HERE? ***********
              CALL BMFLUX(JNTRVL,AASALB,AADREF,S0,SURSCL(1))

!             CHECK IF DISORT SCALING SPECTRAL GRID POINTS:
              IF(NOPRNT.NE.9)THEN

!                 SAVE MS DATA FOR REPEAT USE:
                  IF(.NOT.LJMASS .AND. LSAVMS)                          &
     &              WRITE(ISCRCH)JNTRVL,(COSBAR(ISEG),GTSCAT(0,ISEG),   &
     &               (TAUT(INTRVL,ISEG),INTRVL=1,JNTRVL),ISEG=1,ML-1),  &
     &               ((UPF(INTRVL,ISEG),DNF(INTRVL,ISEG),               &
     &               UPFS(INTRVL,ISEG),DNFS(INTRVL,ISEG),               &
     &               INTRVL=1,JNTRVL),ISEG=1,ML)
                  IF(DODGRD)THEN
                      CALL SCNFLX(VCEN,                                 &
     &              NUM_LRES,SOLAR_FLUX,DIFFUSE_FLUX,LNFLRT,FLRT,       &
     &              NUM_LEVS,FLX_UPDIFFUSE,FLX_DNDIFFUSE,FLX_DNDIRECT)
...
!                 CALCULATE COOLING RATES:
          ...
          ELSE
              ...
              IF(.NOT.LJMASS .AND. LSAVMS)WRITE(ISCRCH)IONE,            &
     &          (COSBAR(ISEG),GTSCAT(0,ISEG),TAUT(1,ISEG),ISEG=1,ML-1), &
     &          (UPF(1,ISEG),DNF(1,ISEG),                               &
     &          UPFS(1,ISEG),DNFS(1,ISEG),ISEG=1,ML)
          ENDIF

!         FOR EACH WAVENUMBER, BMOD0 PERFORMS INITIALIZATIONS FOR BMOD:
          IF(MODTRN)CALL BMOD0(IBINPT,VCEN,.FALSE.,LBMCEN,LBMFLG)

!     VERTICAL PATH COMPLETE.
      ENDIF
...
!         SPECTRAL FLUX OUTPUT AT SENSOR:
          IF(NOPRNT.LE.-1 .AND. .NOT.LUSEMS .AND. .NOT.NO_WRT)THEN
              N=LOWALT(0,ILOS)
              NP1=N+1
...
!                     ARE INACCURATE AND ARE SET TO ZERO.
                      TMOL=TX(17)*UNIF*TX(31)*TRACE*TRANXY
                      IF(TMOL.LT.1.E-8)TMOL=0.
                      CALL SSRAD(IPH,ILOS,ISEG,IPATH,VCEN,S0,           &
     &                  PFMOL0,PFMOL2,TMOL,SSAP,TX(1),                  &
     &                  TSNOBS(ILOS),TSNREF(ILOS),SUMSSS(ILOS))
                      GOTO(40,50),IPATH
                  ENDIF
...
!                 CK SOLAR PATH:
                  ...
                  CALL SSCORK(.FALSE.,IPH,ILOS,ISEG,IPATH,JNTRVL,       &
     &              VCEN,S0,PFMOL0,PFMOL2,SSAP,TX(1),TSNOBS(ILOS),      &
     &              SUMSSS(ILOS),RADCUM)
                  GOTO(40,50),IPATH
              ELSE
...
!                     LINE-OF-SIGHT PATH WITHOUT MOD3D:
                      CALL CKLOS(JNTRVL,ISEG,ILOS)
                  ENDIF
                  IF(IEMSCT.EQ.2)CALL SSCORK(.FALSE.,IPH,ILOS,ISEG,     &
     &              IPATH,JNTRVL,VCEN,S0,PFMOL0,PFMOL2,SSAP,TX(1),      &
     &              TSNOBS(ILOS),SUMSSS(ILOS),RADCUM)
              ENDIF
...
!             TWO-FLUX MULTIPLE SCATTERING.  DEFINE BLACKBODY FUNCTIONS.
              BLAYER=BBFN(TSEG(ISEG,ILOS),VCEN)
              BBOUND=BBFN(PATH_T(ISEG-1,ILOS),VCEN)
              N=MAPPTH(ISEG,ILOS)
              B0=BETABS(REAL(PTHCOS(ISEG-1)),COSBAR(N))
              UP=B0/PI
              DN=(1-B0)/PI
              NFRNTL=LOWALT(ISEG-1,ILOS)
              NFRNTH=NFRNTL+1
...
!                 FLUXES AT FRONT OF PATH SEGMENT:
                  ...
                  IF(FACALT(ISEG-1,ILOS).LE.0.)THEN

!                     INTERPOLATE FLUXES TO PTHALT(ISEG-1,ILOS):
...
!                 FLUXES AT BACK OF PATH SEGMENT:
                  ...
                  IF(FACALT(ISEG,ILOS).LE.0.)THEN

!                     INTERPOLATE FLUXES TO ALTITUDE PTHALT(ISEG,ILOS):
...
!                 THERMAL AND SOLAR SOURCE TERMS.
     ...
     &              +(1-OMEGA)*(BNCOEF*BBOUND+2*DFCOEF*(BLAYER-BBOUND))
                  ...
                  SUMTMS(ILOS)=SUMTMS(ILOS)+WTTRAN*THMSCT
                  SUMMS(ILOS)=SUMMS(ILOS)+WTTRAN*SOLLAY
                  RADCUM(INTRVL)                                        &
     &              =RADCUM(INTRVL)+TRNCUM(INTRVL)*(THMLAY+SOLLAY)
                  RMSCUM(INTRVL)                                        &
     &              =RMSCUM(INTRVL)+TRNCUM(INTRVL)*(THMSCT+SOLLAY)
...
!                 NOT TERMINATE AT A LAYER BOUNDARY.
     ...
     &                      UPFLXS(NBACKL),DNFLXS(NBACKL),SMSOLL,       &
     &                      SUMMS(ILOS))
                      ELSEIF(NLOS.EQ.1)THEN
     ...
     &                  SMSOLL,SUMMS(ILOS))
                  ELSEIF(NLOS.EQ.1)THEN
                      WRITE(IPR1,'(F8.2,F7.3,2F9.6,1X,1P,9E11.4)')VCEN, &
     &                  PTHALT(ISEG,ILOS),TX(9),SMTRNL,NTFLX(NBACKL)+   &
     &                    FACTOR*(NTFLX(NBACKH)-NTFLX(NBACKL)),         &
     &                  UPFLX(NBACKL)+                                  &
     &                    FACTOR*(UPFLX(NBACKH)-UPFLX(NBACKL)),         &
     &                  DNFLX(NBACKL)+                                  &
     ...
     &                  SMSOLL,SUMMS(ILOS)
                  ELSEIF(ILOS.EQ.1)THEN
                      WRITE(IPR1,'(F8.2,I4,F7.3,2F9.6,1X,1P,9E11.4)')   &
     &                  VCEN,ILOS,PTHALT(ISEG,ILOS),TX(9),              &
     &                  SMTRNL,NTFLX(NBACKL)+                           &
     &                    FACTOR*(NTFLX(NBACKH)-NTFLX(NBACKL)),         &
     &                  UPFLX(NBACKL)+                                  &
     &                    FACTOR*(UPFLX(NBACKH)-UPFLX(NBACKL)),         &
     ...
     &                  SMSOLL,SUMMS(ILOS)
                  ELSE
                      WRITE(IPR1,'(I12,F7.3,2F9.6,1X,1P,9E11.4)')ILOS,  &
     &                  PTHALT(ISEG,ILOS),TX(9),SMTRNL,NTFLX(NBACKL)+   &
     &                    FACTOR*(NTFLX(NBACKH)-NTFLX(NBACKL)),         &
     &                  UPFLX(NBACKL)+                                  &
     &                    FACTOR*(UPFLX(NBACKH)-UPFLX(NBACKL)),         &
...
!             K-DISTRIBUTION DEPENDENT OUTPUT:
     ...
     &                  +SUMSSS(ILOS)+(PTHSCL(ILOS)-1)                  &
     &                  *(SUMTMS(ILOS)+SUMMS(ILOS)),((PTHSCL(ILOS)-1)   &
     &                  *RMSCUM(INTRVL)+RADCUM(INTRVL),INTRVL=1,JNTRVL)
                  ELSE
                      WRITE(ITKDIS,TFRMT)                               &
     &                  PTHALT(ISEG,ILOS),PTHRNG(ISEG,ILOS),TX(9),      &
     &                  (TRNCUM(INTRVL),INTRVL=1,JNTRVL)
                      WRITE(IRKDIS,RFRMT)PTHALT(ISEG,ILOS),             &
     &                  PTHRNG(ISEG,ILOS),THMCUM(ILOS)+SUMMS(ILOS)      &
     &                  +SUMSSS(ILOS)+(PTHSCL(ILOS)-1)                  &
     &                  *(SUMTMS(ILOS)+SUMMS(ILOS)),((PTHSCL(ILOS)-1)   &
     &                  *RMSCUM(INTRVL)+RADCUM(INTRVL),INTRVL=1,JNTRVL)
...
!         SCALE ISAACS BY DISORT-ISAACS RATIO
          SUMTMS(ILOS)=PTHSCL(ILOS)*SUMTMS(ILOS)
          SUMMS(ILOS)=PTHSCL(ILOS)*SUMMS(ILOS)
          FDNTRT(ILOS)=SURSCL(ILOS)*FDNTRT(ILOS)*TPHDIR(ILOS)/PI
          FDNRT(ILOS)=FDNTRT(ILOS)                                      &
     &      +SURSCL(ILOS)*FDNRT(ILOS)*TPHDIR(ILOS)/PI
...
!                     SURFACE EMISSION + DIFFUSE THERMAL REFLECTION.
                      WRITE(JRKDIS,REC=NRKDIS)PTHALT(NSEG(ILOS),ILOS),  &
     &                  PTHRNG(NSEG(ILOS),ILOS),GREY*TRNLOS(ILOS)       &
     &                  +FDNTRT(ILOS),(TRNCUM(INTRVL)*(GREY+TPHDIR(ILOS)&
     &                  *SURSCL(ILOS)*DNF(INTRVL,1)/PI),INTRVL=1,JNTRVL)
                  ELSE
...
!                         NO DIRECT SOLAR REFLECTION SURFACE TERM.
     ...
     &                  +SURSCL(ILOS)*(DNF(INTRVL,1)+DNFS(INTRVL,1))    &
