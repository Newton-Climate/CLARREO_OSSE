      SUBROUTINE FNAMES(NMFLRT,CTAPE5FN,CLRT,SPFLUX,DBOUT,REFPTH)
...
!       REFPTH   NAME OF REFRACTIVE PATH INPUT FILE.
      CHARACTER CLRT*(*),SPFLUX*(*),DBOUT*(*),REFPTH*(*), CTAPE5FN*(*)
      COMMON /PRONUM/LABEL
      CHARACTER LABEL*6
...
!         TRUNCATE ".TP5" IF PRESENT IN FLRT:
      ...
      IF(LNFLRT.LE.0)THEN
          CALL OPNFL(IPR,0,'tape6','UNKNOWN','FORMATTED','FNAMES')
          CALL OPNFL(IRD,0,ctape5fn,'OLD','FORMATTED','FNAMES')
          ...
          REFPTH='rfracpth.dat'
      ELSE
          CALL OPNFL(IPR,0,FLRT(1:LNFLRT)//LABEL//'.tp6','UNKNOWN',      &
     &      'FORMATTED','FNAMES')
!       CALL OPNFL(IRD,0,FLRT(1:LNFLRT)//'.tp5','OLD','FORMATTED',        &
          CALL OPNFL(IRD,0,ctape5fn,'OLD','FORMATTED', 'FNAMES')
          CALL OPNFL(IPU,0,FLRT(1:LNFLRT)//LABEL//'.tp7','UNKNOWN',     &
     &      'FORMATTED','FNAMES')
!SSI      CALL OPNFL(IDBIN,0,                                           &
!SSI &      FLRT(1:LNFLRT)//LABEL//'.dbi','UNKNOWN','FORMATTED','FNAMES')
          DBOUT=FLRT(1:LNFLRT)//LABEL//'.dbo'
          CLRT=FLRT(1:LNFLRT)//LABEL//'.clr'
          SPFLUX=FLRT(1:LNFLRT)//LABEL//'.flx'
          CHNOUT=FLRT(1:LNFLRT)//LABEL//'.chn'
          LENCHN=LNFLRT+4
          M3DGEN=FLRT(1:LNFLRT)//LABEL//'.g3d'
          M3DCNT=FLRT(1:LNFLRT)//LABEL//'.c3d'
          M3DDAT=FLRT(1:LNFLRT)//LABEL//'.d3d'
          M3DMOL=FLRT(1:LNFLRT)//LABEL//'.m3d'
          REFPTH=FLRT(1:LNFLRT)//LABEL//'.pth'
      ENDIF
      RETURN
      END
...
!     ARGUMENTS:
      INTEGER LNFLRT
      CHARACTER PLTOUT*(*),FLRT*(NAMLEN-4),LABEL*6
      COMMON /PRONUM/LABEL
...
!             BINARY FILES:
              PLTOUT=FLRT(1:LNFLRT)//LABEL//'b.plt'
              INQUIRE(JPU,OPENED=LOPEN)
              IF(.NOT.LOPEN)THEN
                  CALL OPNFL(JPU,0,FLRT(1:LNFLRT)//LABEL//'.tp7',       &
     &               'UNKNOWN','UNFORMATTED','FNAMBN')
                  WRITE(JPU)VERS
              ENDIF
          ELSE
              PLTOUT=FLRT(1:LNFLRT)//LABEL//'.plt'
          ENDIF
