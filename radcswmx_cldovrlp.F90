#include <misc.h>
#include <params.h>
subroutine radcswmx_cldovrlp(lchnk   ,ncol    ,                            &
                    ...
                    cldtau_ice, cldtau_liq                      , &
                    wgtv_v  ,totwgt_v,ccon_v  ,nconfig_v,         &
#ifndef OFFLINE
                    fcns    ,cldovrlp)
#else
                    fcns    ,no_o2_abs,cldovrlp)
#endif
...
!  real(r8), intent(out) :: aerfwd(pcols,nspint,naer_groups) ! Aerosol column averaged forward scattering
   ...
   real(r8), intent(out) :: cldtau_ice(pcols)  ! cloud tau ice (DRF)
   real(r8), intent(out) :: cldtau_liq(pcols)  ! cloud tau liquid (DRF)

! 
! Cloud configurations
! 
   real(r8), intent(out) :: wgtv_v(pcols,nconfgmax)        ! Fractional area for cloud configurations
   real(r8), intent(out) :: totwgt_v(pcols)                ! Sum of wgtv_v = total fractional area
   integer, intent(out) ::  ccon_v(pcols,pver,nconfgmax)   ! Binary cloud configurations (0=no,1=yes)
   integer, intent(out) ::  nconfig_v(pcols)               ! # of cloud configurations in CAM columns
   logical, intent(in)  ::  cldovrlp                       ! just generate cloud overlap data (no RT calculations)

#ifdef OFFLINE
   logical, intent(in)   :: no_o2_abs          ! Turn off O2 absorption
#endif
! 
!---------------------------Local variables-----------------------------
! 
...
!  write (6, '(a, x, i3)') 'radcswmx : chunk identifier', lchnk
   ...
   do i=1, ncol
!
! Initialize cloud properties for output
!
      nconfig_v(i) =  0
      totwgt_v(i) =   0.0_r8
      wgtv_v(i,:) =   0.0_r8
...
! 
                   ...
                   !DRF reporting cloud optical depths
                   if (ns==1) then
                     cldtau_ice(i) =  cldtau_ice(i) + tauxci(i,k)
                     cldtau_liq(i) =  cldtau_liq(i) + tauxcl(i,k)
                   endif
                   !END DRF
...
!    if (i.eq.122) then
!       write(*,*) 'radcswmx aerssa = ',aerssa(i,:,2)
!       write(*,*) 'radcswmx aerasym = ',aerasm(i,:,2)
!    endif
...
! 
         ...
         ccon(pverp,:) = 0
!
! Record cloud properties for output
!
         nconfig_v(i) = nconfig
         totwgt_v(i) = totwgt
         wgtv_v(i,1:nconfig) = wgtv(1:nconfig)
         ccon_v(i,:,1:nconfig) = ccon(1:pver,1:nconfig)
         if (.not.cldovrlp) then
! 
...
!!! More ugliness to replace earlier ugliness
!!! This will be replaced with better code during the merge of the vector branch
                do k = 1,pverp
                   rdenom = 1._r8/(1._r8 - rdndifc(k)*rupdifc(k))
                   fcns(i,k)=fcns(i,k) + solflx*( & ! fluxdn is inserted here
                        (exptdnc(k) + & 
                        (tdntotc(k) - exptdnc(k) + exptdnc(k)*rupdirc(k)*rdndifc(k))* &
                        rdenom & ! end of fluxdn
                        ) - ( & ! fluxup is inserted here
                        (exptdnc(k)*rupdirc(k) + (tdntotc(k)-exptdnc(k))*rupdifc(k))* &
                        rdenom) & ! end of fluxup
                        )
                enddo
! 
...
!  write (6, '(a, x, i3)') 'radcswmx : exiting, chunk identifier', lchnk
   ...
   return
end subroutine radcswmx_cldovrlp
