     ...
     &  KNTRVL,LASTM,LDASTM,NSSALB,RHASYM,KPRINT,                       &
...
!     INPUT CARD1A:
     ...
     &      H2OAER,CDTDIR,SOLCON,CDASTM,ASTMC,ASTMX,ASTMO,AERRH,NSSALB
          ELSE
          READ(TP5_FILE(TP5_LINENUMBER),'(A3,I2,A1,F4.0,F10.0,2A10,2A1, &
     &          4(1X,A1),F10.0,A1,                                      &
     &      F9.0,3F10.0,I10)',IOSTAT=IOS)DISSTR,NSTR,CSTR,SFWHM,CO2MX,  &
     &      H2OSTR,O3STR,C_PROF,USRSUN(1:1),BMROOT(1:1),FILTNM(1:1),    &
     &      H2OAER,CDTDIR,SOLCON,CDASTM,ASTMC,ASTMX,ASTMO,AERRH,NSSALB
...
!             USRSUN IS READ IN AND USED EVEN IF SFWHM=0.
              IF(TP5_FLAG) THEN  !DRF MODIFICATION
                 READ(IRD,FRMT)USRSUN
              ELSE
                 READ(TP5_FILE(TP5_LINENUMBER),FRMT)USRSUN
                 TP5_LINENUMBER=TP5_LINENUMBER+1
              ENDIF
              LSUN=.TRUE.
...
!         CARD 1A2:
          IF(BMROOT(1:1).EQ.'T' .OR. BMROOT(1:1).EQ.'t')THEN
              ...
              ELSE
                 READ(TP5_FILE(TP5_LINENUMBER),FRMT)BMROOT
                 TP5_LINENUMBER=TP5_LINENUMBER+1
              ENDIF
          ELSE
              BMROOT=' '
...
!         CARD 1A3:
          IF(FILTNM(1:1).EQ.'T' .OR. FILTNM(1:1).EQ.'t')THEN
              ...
              ELSE
                 READ(TP5_FILE(TP5_LINENUMBER),FRMT)FILTNM
                 TP5_LINENUMBER=TP5_LINENUMBER+1
              ENDIF
          ELSE
              FILTNM=' '
...
!         CARD 1A4:
          IF(CDTDIR.EQ.'T' .OR. CDTDIR.EQ.'t')THEN
              ...
              ELSE
                 READ(TP5_FILE(TP5_LINENUMBER),FRMT)DATDIR
                 TP5_LINENUMBER=TP5_LINENUMBER+1
              ENDIF
          ENDIF
...
!         CARD 1A5:
          IF(L_UMIX)THEN
              ...
              ELSE
                 READ(TP5_FILE(TP5_LINENUMBER),'(10F5.0)',IOSTAT=IOS)   &
     &                S_UMIX
                 TP5_LINENUMBER=TP5_LINENUMBER+1
              ENDIF
              IF(IOS.NE.0)THEN
...
!         CARD 1A6:
          IF(L_XSEC)THEN
              ...
              ELSE
                 READ(TP5_FILE(TP5_LINENUMBER),'(15F5.0)',IOSTAT=IOS)   &
     &                S_XSEC
                 TP5_LINENUMBER=TP5_LINENUMBER+1
              ENDIF
              IF(IOS.NE.0)THEN
...
!         CARD 1A7:
          IF(L_TRAC)THEN
              ...
              ELSE
                 READ(TP5_FILE(TP5_LINENUMBER),'(16F5.0)',IOSTAT=IOS)   &
     &                S_TRAC
                 TP5_LINENUMBER=TP5_LINENUMBER+1
              ENDIF
              IF(IOS.NE.0)THEN
     ...
     &              WRITE(IPR,'(A,F9.5)')YNAM16(IMOL),S_TRAC(IMOL)
      ...
      CALL RSSALB(NSSALB,RHASYM,TP5_FLAG,TP5_FILE,TP5_LINENUMBER)

!     OPEN MOLECULAR BAND MODEL PARAMETERS FILE AND READ HEADER:
      KNTRVL=1
      write(*,*) 'Card 1A' , BMROOT
      write(*,*) 'DATDIR = ',DATDIR(1:LENDAT)
      IF(MODTRN)THEN

!         OPEN BAND MODEL DATA FILES:
          CALL OPENBM(LENDAT,DATDIR(1:LENDAT),BMROOT,RESCHR)
          write(*,*), 'in Card 1A'
          write(*,*), DATDIR
         write(*,*), BMROOT
