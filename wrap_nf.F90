...
! Author: Jim Rosinski
!
! $Id: wrap_nf.F90,v 1.1.2.1 2004/03/26 22:01:28 mvertens Exp $
!
!-------------------------------------------------------------------------------
...
!===============================================================================
   ...
   subroutine wrap_get_att_real (nfid, varid, attname, attreal)
!-------------------------------------------------------------------------------
!
! Purpose:
!
! Returns the attribute real from the given variable ID and attribute name
!
!-------------------------------------------------------------------------------
...
#include <netcdf.inc>
   ...
   integer, intent(in):: varid
   character*(*), intent(in):: attname
   real(r4), intent(out):: attreal

   integer ret      ! NetCDF return code

   ret = nf_get_att_real (nfid, varid, attname, attreal)
   if (ret/=NF_NOERR) call handle_error (ret)
   end subroutine wrap_get_att_real
...
#endif
   if (ret/=NF_NOERR) then
     write(6,*)'WRAP_GET_VAR_REAL4: error reading varid =', varid
     call handle_error (ret)
   end if
...
!===============================================================================
   ...
   subroutine wrap_get_vara_real4 (nfid, varid, start, count, arr)
!-------------------------------------------------------------------------------
!
! Purpose:
!
...
#endif
   if (ret/=NF_NOERR) then
     write(6,*)'WRAP_GET_VARA_REAL4: error reading varid =', varid
     call handle_error (ret)
   end if
   end subroutine wrap_get_vara_real4
